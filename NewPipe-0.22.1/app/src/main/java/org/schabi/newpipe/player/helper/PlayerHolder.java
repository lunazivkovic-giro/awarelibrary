...
public final class PlayerHolder {
    ...
    public static final MainAware AWARE = new MainAware();
    /**
    ...
    public void stopService() {
        ...
        MainActivity.timeStopVideo.add(Double.valueOf(System.currentTimeMillis()));
        // Aware.stopLocations(context);
        //Aware.stopBattery(context);
        // Aware.stopPlugin(context, "com.aware.plugin.google.activity_recognition");
        //Aware.stopLight(context);
        try {
            AWARE.getData(getCommonContext(), MainActivity.timeStart);
        } catch (final InterruptedException e) {
            e.printStackTrace();
        }

        MainActivity.timeStop = System.currentTimeMillis();

        //for (int i = 0; i < MainActivity.timeStartVideo.size(); i++) {

        // MainActivity.totalTime += MainActivity.timeStopVideo.get(i) -
        // MainActivity.timeStartVideo.get(i);

        // }


        MainActivity.totalTimeInApp = MainActivity.timeStop - MainActivity.timeStart;

        System.out.println("Total in app" + MainActivity.totalTimeInApp);
